...
function ac_select() {
    if ( $("#ac_select option:selected").val() != "") {
            if( $('#ac_select').size() >= 1){ //don't hide the address if we just have "Create new accounts"
                $(".ac_ad").hide(); 
            }//else{ alert('you have selected:' + $("#ac_select option:selected").val() + ' from a list of ' + $('#ac_select').size()); }
...
$(document).ready(function() {
   $('#email').focus(); 
    ...
    if( ( $('#ac_select option').size() > 1 ) && $("#ac_select option:selected").val() != ""){ //don't hide the address if we just have "Create new accounts"
        $(".ac_ad").hide();
    }else if($("#ac_select option:selected").val() != ""){
        $(".ac_ad").hide();
    }else{
        //$('#ac_select').attr('disabled', true);
        $('#ac_select').attr('hidden', true);
    }
